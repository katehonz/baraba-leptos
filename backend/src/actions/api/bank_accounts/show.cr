class Api::BankAccounts::Show < ApiAction
  get "/api/companies/:company_id/bank_accounts/:bank_account_id" do
    account = BankAccountQuery.new.company_id(company_id).find(bank_account_id)

    gl_account = if gl_id = account.gl_account_id
                   AccountQuery.new.id(gl_id).first?
                 end
    buffer_account = if buf_id = account.buffer_account_id
                       AccountQuery.new.id(buf_id).first?
                     end

    json({
      success: true,
      data:    {
        id:                   account.id,
        name:                 account.name,
        iban:                 account.iban,
        bic:                  account.bic,
        currency:             account.currency,
        bank_name:            account.bank_name,
        integration_type:     account.integration_type,
        gl_account_id:        account.gl_account_id,
        gl_account_code:      gl_account.try(&.code),
        gl_account_name:      gl_account.try(&.name),
        buffer_account_id:    account.buffer_account_id,
        buffer_account_code:  buffer_account.try(&.code),
        buffer_account_name:  buffer_account.try(&.name),
      },
    })
  end
end
