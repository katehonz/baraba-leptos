# Show a single VAT return
class Api::VatReturns::Show < ApiAction
  include Api::Auth::SkipRequireAuthToken

  get "/api/companies/:company_id/vat_returns/:vat_return_id" do
    ...
    if vat_return
      result = vat_return.result_json ? JSON.parse(vat_return.result_json.not_nil!) : JSON::Any.new({} of String => JSON::Any)
      vat_due = result["vat_due"]?.try(&.as_f?) || 0.0

      json({
        success: true,
        data:    vat_return_json(vat_return, vat_due),
      })
    else
      json({success: false, error: "VAT return not found"})
    end
  end

  private def vat_return_json(vr : VatReturn, vat_due : Float64)
    {
      id:                   vr.id,
